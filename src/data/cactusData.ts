export interface CactusSpecies {
  id: string;
  name: string;
  scientificName: string;
  family: string;
  description: string;
  origin: string;
  care: {
    watering: string;
    light: string;
    temperature: string;
    soil: string;
  };
  image: string;
  blooming: string;
  size: string;
  difficulty: 'Легкий' | 'Средний' | 'Сложный';
}

export const cactusData: CactusSpecies[] = [
  {
    id: '1',
    name: 'Сагуаро',
    scientificName: 'Carnegiea gigantea',
    family: 'Cactaceae',
    description: 'Величественный гигант пустыни Сонора, символ американского Юго-Запада. Может достигать высоты до 12 метров и жить более 150 лет.',
    origin: 'Пустыня Сонора, США и Мексика',
    care: {
      watering: 'Редкий полив раз в 2-3 недели летом',
      light: 'Яркий прямой солнечный свет',
      temperature: '18-35°C, минимум 10°C зимой',
      soil: 'Хорошо дренированная кактусовая смесь'
    },
    image: '/img/5785449a-ab71-44ac-a472-c37c9bcb887b.jpg',
    blooming: 'Май-июнь, белые цветы',
    size: 'До 12 м в высоту',
    difficulty: 'Средний'
  },
  {
    id: '2',
    name: 'Бочковый кактус',
    scientificName: 'Ferocactus cylindraceus',
    family: 'Cactaceae',
    description: 'Округлый кактус с мощными колючками и яркими цветами. Отличный накопитель воды, способен выживать в экстремальных условиях.',
    origin: 'Пустыни Мохаве и Сонора',
    care: {
      watering: 'Умеренный полив раз в 3-4 недели',
      light: 'Яркое освещение, переносит полутень',
      temperature: '15-30°C, устойчив к заморозкам',
      soil: 'Песчано-гравийная смесь'
    },
    image: '/img/50ad3ef9-6be6-420a-87a5-cacb1a368e35.jpg',
    blooming: 'Июль-сентябрь, желтые цветы',
    size: '1-3 м в высоту',
    difficulty: 'Легкий'
  },
  {
    id: '3',
    name: 'Опунция',
    scientificName: 'Opuntia ficus-indica',
    family: 'Cactaceae',
    description: 'Плоский кактус с съедобными плодами, широко культивируется в Средиземноморье. Один из самых адаптивных видов кактусов.',
    origin: 'Мексика, натурализована по всему миру',
    care: {
      watering: 'Регулярный полив весной и летом',
      light: 'Полное солнце или легкая полутень',
      temperature: '5-40°C, морозостойкая',
      soil: 'Любая хорошо дренированная почва'
    },
    image: 'https://cdn.poehali.dev/files/28261965-19e5-4eaf-add8-2feec1a377cb.jpg',
    blooming: 'Июнь-август, желтые цветы',
    size: '2-5 м в высоту',
    difficulty: 'Легкий'
  },
  {
    id: '4',
    name: 'Рождественский кактус',
    scientificName: 'Schlumbergera truncata',
    family: 'Cactaceae',
    description: 'Эпифитный кактус-суккulent из тропических лесов Бразилии. Популярное комнатное растение, цветущее зимой яркими цветами.',
    origin: 'Атлантические леса Бразилии',
    care: {
      watering: 'Регулярный полив, не допускать пересыхания',
      light: 'Яркий рассеянный свет, избегать прямого солнца',
      temperature: '15-21°C, прохладный период для цветения',
      soil: 'Рыхлая, хорошо дренированная почва для эпифитов'
    },
    image: '/img/a13fce8a-f444-49f3-a129-50dac170c908.jpg',
    blooming: 'Ноябрь-январь, розовые, красные цветы',
    size: '30-60 см в диаметре',
    difficulty: 'Легкий'
  },
  {
    id: '5',
    name: 'Органная труба',
    scientificName: 'Stenocereus thurberi',
    family: 'Cactaceae',
    description: 'Величественный колонновидный кактус с множественными стволами. Получил название из-за сходства с органными трубами.',
    origin: 'Пустыня Сонора, Мексика и Аризона',
    care: {
      watering: 'Минимальный полив раз в месяц',
      light: 'Полное солнце, очень светолюбив',
      temperature: '20-40°C, не переносит заморозки',
      soil: 'Каменистая, хорошо дренированная почва'
    },
    image: '/img/990602a5-090a-41df-aad7-7caf177eeb7d.jpg',
    blooming: 'Апрель-июнь, белые или розовые цветы',
    size: '6-8 м в высоту',
    difficulty: 'Сложный'
  },
  {
    id: '6',
    name: 'Старик',
    scientificName: 'Cephalocereus senilis',
    family: 'Cactaceae',
    description: 'Уникальный кактус, покрытый длинными белыми волосками, защищающими от солнца и холода. Символ мексиканской флоры.',
    origin: 'Центральная Мексика, штат Идальго',
    care: {
      watering: 'Очень редкий полив раз в 4-6 недель',
      light: 'Яркое солнце, переносит полутень',
      temperature: '18-30°C, минимум 5°C',
      soil: 'Известковая, щелочная почва с отличным дренажем'
    },
    image: '/img/30c6f9ec-da18-4a41-bdaa-0706f77379d6.jpg',
    blooming: 'Редко цветет в культуре, розовые цветы',
    size: '12-15 м в природе, до 2 м дома',
    difficulty: 'Средний'
  },
  {
    id: '7',
    name: 'Золотая бочка',
    scientificName: 'Echinocactus grusonii',
    family: 'Cactaceae',
    description: 'Сферический кактус с золотистыми колючками. Один из самых популярных декоративных кактусов, символ мексиканских пустынь.',
    origin: 'Центральная Мексика',
    care: {
      watering: 'Редкий полив раз в 3-4 недели летом',
      light: 'Яркий прямой солнечный свет',
      temperature: '20-35°C, минимум 8°C',
      soil: 'Песчаная кактусовая смесь с добавлением перлита'
    },
    image: '/img/dbba7852-b254-44ac-be1b-7d8661608c56.jpg',
    blooming: 'Июнь-сентябрь, желтые цветы на верхушке',
    size: 'До 1,3 м в диаметре',
    difficulty: 'Легкий'
  },
  {
    id: '8',
    name: 'Царица ночи',
    scientificName: 'Epiphyllum oxypetalum',
    family: 'Cactaceae',
    description: 'Эпифитный кактус с огромными ароматными цветами, раскрывающимися только ночью. Одно из самых эффектных цветений в мире кактусов.',
    origin: 'Тропические леса Центральной Америки',
    care: {
      watering: 'Обильный полив в период роста, зимой сократить',
      light: 'Яркий рассеянный свет, избегать прямого солнца',
      temperature: '18-24°C, повышенная влажность',
      soil: 'Рыхлая эпифитная смесь с корой и мхом'
    },
    image: '/img/f9bef502-4863-4bfd-83c7-77ad0573b8ef.jpg',
    blooming: 'Лето, огромные белые цветы ночью',
    size: '2-3 м в длину плетей',
    difficulty: 'Средний'
  },
  {
    id: '9',
    name: 'Епископская шапка',
    scientificName: 'Astrophytum myriostigma',
    family: 'Cactaceae',
    description: 'Звездообразный кактус без колючек с характерными белыми пятнами. Форма напоминает епископскую митру, отсюда и название.',
    origin: 'Северо-восточная Мексика',
    care: {
      watering: 'Очень осторожный полив раз в 3-4 недели',
      light: 'Яркое солнце, но защищать от полуденного зноя',
      temperature: '22-28°C, зимой 10-15°C',
      soil: 'Минеральная смесь с большим содержанием песка'
    },
    image: '/img/943b2e6b-c3aa-4056-baaf-debce8636624.jpg',
    blooming: 'Лето, желтые цветы на верхушке',
    size: '15-20 см в диаметре',
    difficulty: 'Сложный'
  },
  {
    id: '10',
    name: 'Айлостера белоцветковая',
    scientificName: 'Aylostera albiflora',
    family: 'Cactaceae',
    description: 'Миниатюрный шаровидный кактус из высокогорий Боливии. Образует плотные группы и обильно цветет белоснежными цветками весной.',
    origin: 'Высокогорья Боливии, Анды',
    care: {
      watering: 'Осторожный полив в период роста, зимой сухое содержание',
      light: 'Яркий свет с притенением от полуденного солнца',
      temperature: '15-25°C летом, 5-10°C зимой для стимуляции цветения',
      soil: 'Очень дренированная минеральная смесь с песком'
    },
    image: '/img/db349199-55d7-4136-8a92-9a721b80b8e8.jpg',
    blooming: 'Март-май, белые цветы диаметром до 3 см',
    size: '3-5 см в диаметре',
    difficulty: 'Средний'
  },
  {
    id: '11',
    name: 'Эхинофоссулокактус',
    scientificName: 'Echinofossulocactus multicostatus',
    family: 'Cactaceae',
    description: 'Уникальный кактус с волнистыми ребрами и мощными центральными колючками. Известен своими причудливо изогнутыми ребрами, которые создают неповторимый рельеф.',
    origin: 'Центральная Мексика, пустыни Чиуауа',
    care: {
      watering: 'Умеренный полив весной и летом, зимой практически без воды',
      light: 'Яркое солнечное освещение, переносит прямые лучи',
      temperature: '20-30°C летом, 8-12°C зимой',
      soil: 'Кактусовая смесь с добавлением крупного песка и перлита'
    },
    image: 'https://cdn.poehali.dev/files/3324cd0e-b44b-468f-8e13-1de5b2c6128f.jpg',
    blooming: 'Весна-лето, мелкие розовые или белые цветы',
    size: '8-15 см в диаметре',
    difficulty: 'Средний'
  },
  {
    id: '12',
    name: 'Астрофитум Мириостигма',
    scientificName: 'Astrophytum myriostigma',
    family: 'Cactaceae',
    description: 'Знаменитый кактус "епископская шапка" без колючек, покрытый белыми крапинками. Один из самых узнаваемых и популярных кактусов среди коллекционеров.',
    origin: 'Центральная и Северная Мексика',
    care: {
      watering: 'Редкий полив в период роста, зимой без воды',
      light: 'Яркое солнечное освещение круглый год',
      temperature: '22-28°C летом, 10-15°C зимой',
      soil: 'Очень дренированная минеральная смесь, много песка'
    },
    image: '/img/4dfd20ac-561c-4db8-bf4a-862f21532c74.jpg',
    blooming: 'Лето, крупные желтые цветы с красным центром',
    size: '15-20 см в диаметре',
    difficulty: 'Легкий'
  },
  {
    id: '13',
    name: 'Гимнокалициум',
    scientificName: 'Gymnocalycium mihanovichii',
    family: 'Cactaceae',
    description: 'Компактный шаровидный кактус из Южной Америки с изящными изогнутыми колючками. Популярен среди новичков благодаря неприхотливости и обильному цветению.',
    origin: 'Аргентина, Парагвай, Южная Бразилия',
    care: {
      watering: 'Регулярный полив весной и летом, зимой сократить',
      light: 'Яркий рассеянный свет, притенение от прямого солнца',
      temperature: '18-25°C летом, 12-15°C зимой',
      soil: 'Кактусовая смесь с хорошим дренажем'
    },
    image: '/img/d03de8cd-ed39-4dcf-ba21-38d0b9ec41f9.jpg',
    blooming: 'Весна-осень, розовые, белые или желтые цветы',
    size: '5-10 см в диаметре',
    difficulty: 'Легкий'
  },
  {
    id: '14',
    name: 'Гимнокалициум альбоареолатум',
    scientificName: 'Gymnocalycium alboareolatum',
    family: 'Cactaceae',
    description: 'Компактный кактус с характерными белыми ареолами и изогнутыми колючками. Отличается темно-зеленым телом с контрастными белыми колючковыми подушечками.',
    origin: 'Аргентина, провинция Сальта',
    care: {
      watering: 'Умеренный полив весной и летом, зимой сухое содержание',
      light: 'Яркий рассеянный свет, притенение от жаркого солнца',
      temperature: '18-28°C летом, 10-15°C зимой',
      soil: 'Минеральная кактусовая смесь с хорошим дренажем'
    },
    image: 'https://cdn.poehali.dev/files/128c497f-d782-4461-b104-96c003e45286.jpg',
    blooming: 'Весна-лето, белые или розоватые цветы',
    size: '6-12 см в диаметре',
    difficulty: 'Средний'
  },
  {
    id: '15',
    name: 'Гимнокалициум амбатоенсе',
    scientificName: 'Gymnocalycium ambatoense',
    family: 'Cactaceae',
    description: 'Редкий высокогорный кактус с крупными белыми цветами и мощными изогнутыми колючками. Отличается исключительно красивым цветением и компактными размерами.',
    origin: 'Боливия, департамент Кочабамба',
    care: {
      watering: 'Осторожный полив в период роста, зимой полная засуха',
      light: 'Яркий свет с притенением от полуденного солнца',
      temperature: '15-25°C летом, 5-12°C зимой для стимуляции цветения',
      soil: 'Очень дренированная минеральная смесь с большим содержанием песка'
    },
    image: 'https://cdn.poehali.dev/files/ee012037-67ab-4014-986d-b45b1b3faa92.jpg',
    blooming: 'Весна-лето, крупные белые цветы до 6 см в диаметре',
    size: '8-15 см в диаметре',
    difficulty: 'Сложный'
  },
  {
    id: '16',
    name: 'Сулькоребуция Аренацея',
    scientificName: 'Sulcorebutia arenacea',
    family: 'Cactaceae',
    description: 'Миниатюрный высокогорный кактус с потрясающими оранжевыми цветами, образующими яркий венок вокруг спирально закрученного тела. Один из самых красивоцветущих представителей рода.',
    origin: 'Высокогорья Боливии, Анды (3000-4000 м)',
    care: {
      watering: 'Очень осторожный полив весной и летом, зимой полное высыхание',
      light: 'Яркий свет с притенением от полуденного солнца',
      temperature: '15-25°C летом, 5-10°C зимой для стимуляции цветения',
      soil: 'Минеральная смесь с отличным дренажем, много песка и перлита'
    },
    image: 'https://cdn.poehali.dev/files/8c6045e0-95eb-45be-83f1-e10de8fdf967.jpg',
    blooming: 'Весна-начало лета, множественные оранжевые цветы',
    size: '3-6 см в диаметре',
    difficulty: 'Сложный'
  },
  {
    id: '17',
    name: 'Астрофитум Мириостигма Крапчатый',
    scientificName: 'Astrophytum myriostigma var. quadricostatum',
    family: 'Cactaceae',
    description: 'Классическая форма "епископской шапки" с четко выраженными белыми крапинками, покрывающими всю поверхность. Отличается от базового вида более плотным покрытием трихом и квадратной формой.',
    origin: 'Центральная Мексика, штаты Сан-Луис-Потоси и Идальго',
    care: {
      watering: 'Редкий полив в период роста, зимой полная засуха',
      light: 'Яркое солнечное освещение, переносит прямые лучи',
      temperature: '22-30°C летом, 8-12°C зимой',
      soil: 'Очень дренированная минеральная смесь с известняком'
    },
    image: 'https://cdn.poehali.dev/files/4c83ed2d-ad82-489b-91f7-019e49e49ccd.jpg',
    blooming: 'Лето, крупные ярко-желтые цветы до 7 см в диаметре',
    size: '12-20 см в диаметре',
    difficulty: 'Легкий'
  }
];